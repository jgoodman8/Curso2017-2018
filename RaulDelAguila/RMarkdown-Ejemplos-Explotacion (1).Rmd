...
Vamos a identificar, por tipo de contrato, los proveedores únicos. Los contratos de suministros y servicios tienen un límite máximo de 18.000 y los de obras 50.000. Vamos a analizar aquellos *proveedores únicos* que son *outliers*.
...
print(obras.cercanas.umbral)
print(servysum.cercanos.umbral)
```
...
print(obras.superiores.umbral)
print(servysum.superiores.umbral)
...
##Outliers en agregación de contratos

Vamos a ver si existen outliers en las contrataciones recurrentes, tanto por imoprte como por número de contrataciones:

```{r}

obras.sup.outliers.n<-vector_es_outlier_IQR(obras.superiores.umbral,3)
servysum.sup.outliers.n<-vector_es_outlier_IQR(servysum.superiores.umbral,3)

any((obras.sup.outliers.n)==TRUE)
any((servysum.sup.outliers.n)==TRUE)

obras.sup.outliers.c<-vector_es_outlier_IQR(obras.superiores.umbral,4)
servysum.sup.outliers.c<-vector_es_outlier_IQR(servysum.superiores.umbral,4)

any((obras.sup.outliers.c)==TRUE)
any((servysum.sup.outliers.c)==TRUE)



```

Los outliers son los siguientes:

```{r}
print("outliers de contrataciones de obras por número de contrataciones")
print(obras.superiores.umbral[obras.sup.outliers.n,])

print("outliers de contrataciones de obras por importe de contrataciones")
print(obras.superiores.umbral[obras.sup.outliers.c,])

print("outliers de servicios de obras por número de contrataciones")
print(servysum.superiores.umbral[servysum.sup.outliers.n,])

print("outliers de servicios de obras por importe de contrataciones")
print(servysum.superiores.umbral[servysum.sup.outliers.c,])

```

Recordemos que estos análisis deberían complementarse con otros, y con otras fuentes de información. No obstante, es destacable que sólo 1 consulta de SPARQL ya nos proporciona información útil para comenzar un análisis sobre contratación de proveedores en el ámbito de contrataciones menores y sus características.
